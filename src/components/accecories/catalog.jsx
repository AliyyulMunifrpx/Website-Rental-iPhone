...
function AccessoryCard({ url, name, price, index }) {
      ...
      >
        ...
        <div className="flex gap-4 w-full justify-between items-center">
          <div className="flex flex-col items-start">
            <p className="text-sm lg:text-md text-[#101010]/80">24 jam</p>
          ...
          <span className="px-4 py-2 whitespace-nowrap text-center rounded-full bg-[#101010] hover:bg-[#101010]/90 text-white text-xs lg:text-md">
            Lihat Detail
          </span>
        </div>
      </Link>
...
export default function AksesorisCatalog() {
  const [search, setSearch] = useState("");

  const filteredAccessories = accecories.filter((item) =>
  ...
  return (
    <section className="h-auto w-full flex flex-col mt-24 justify-start items-center px-4 lg:px-16 gap-16">
      <div className="w-full flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        <h2 className="text-[#101010] text-2xl lg:text-4xl text-center lg:text-start">
          Katalog <span className="font-bold">Aksesoris</span>
        </h2>
      ...
      {filteredAccessories.length > 0 ? (
        ...
        <div className="w-full py-20 text-center">
          <p className="text-[#101010]/60">
            Aksesoris yang kamu cari tidak ditemukan.
          </p>
        </div>
      )}
    </section>
